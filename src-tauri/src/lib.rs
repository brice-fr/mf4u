...
async fn start_export(
    ...
    split_period_s: Option<f64>,
    split_first_offset_s: Option<f64>,
    sidecar: tauri::State<'_, Mutex<Sidecar>>,
) -> Result<Value, String> {
    rpc_call(
        &sidecar,
        ...
        json!({
            ...
            "split_mode":           split_mode.unwrap_or_else(|| "none".to_string()),
            "split_size_mb":        split_size_mb.unwrap_or(100.0),
            "split_period_s":       split_period_s.unwrap_or(60.0),
            "split_first_offset_s": split_first_offset_s.unwrap_or(0.0),
        }),
    )
    .await
}
